...
Telepathine.prototype.sendMessage = function (m, address, port) {
	...
	if ((udp) && (b.length < udpMaxMessageSize))  {
		this.udpServer.send(
			b,
			0, // Buffer offset
			b.length,
			...
			function (error, byteLength) {
				if (debug.enabled) debug('gossip:udp %s => %s, type %s, %s bytes', self.peer_name, (address + ':' + port), m.type, b.length)
			}
		);
		return;
	}
	...
	gosipeeSocket.on('data', function (msg) {
		if (debug.enabled) debug('gossip:tcp:data %s => %s, type %s, %s bytes', self.peer_name, peer.name, (msg.type + ''), JSON.stringify(msg).length)
		self.handleMessage(gosipeeSocket, msg, peer)
	});

	// when we are connected, send a request message
	gosipeeSocket.on('connect', function () {
		if (debug.enabled) debug('gossip:tcp:connect %s => %s, type %s, (%s bytes sent)', self.peer_name, address+':'+port, m.type, b.length)
