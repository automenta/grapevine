...
Telepathine.prototype.gossipToPeer = function (peer) {
	var resolvedLocalAddress = this.resolveAddress(peer.address);
	
	if ( (this.port == peer.port) && ( (this.address == peer.address) || (resolvedLocalAddress == this.address)) ) {
		//console.error('gossiping to self');
		return;
	}
	if (debug.enabled) debug('gossip %s %s => %s %s', resolvedLocalAddress, this.port, peer.address, peer.port)
