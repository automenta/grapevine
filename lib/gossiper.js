...
var Gossiper = exports.Gossiper = function (port, seeds, address, emitUpdateOnHeartBeat) {
...
Gossiper.prototype.start = function (callback) {
	...
	this.on('update', function (peer, k, v) {
...
Gossiper.prototype.say = function (event, data, ttl /* in milliseconds*/, buffered) {
	...
	this.setLocalState('event:' + event + ':' + eventID, data, Date.now() + ttl);	
...
Gossiper.prototype.listenToPeer = function (peer) {
	...
	peer.on('update', function (k, v, ttl) {
		...
		if (k !== PeerState.heartbeat)
			self.emit('update', peerName, k, v, ttl);
		else if (self.emitUpdateOnHeartBeat)
			self.emit('update', peerName, k, v, ttl); // heartbeats are disabled by default but it can be changed so this takes care of that
	});
...
Gossiper.prototype.listenToExpiredKeys = function (peer) {
	...
	peer.on('expire', function (k, v, ttl) {
		self.emit('expire', self.my_state.name, k, v, ttl);
	});
}
...
Gossiper.prototype.setLocalState = function (k, v, expiresAt) {
