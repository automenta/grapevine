var
	...
	dns = require('dns'),
	debug = require('debug')('grapevine_gossiper')
...
*/
var Gossiper = exports.Gossiper = function (port, seeds, options) {
...
Gossiper.prototype.start = function (callback) {
	...
	if (udp) {
		...
		udpServer.on("message", function (message, r /* request info */ ) {
			try {
				var mjson = message.toString('utf8');
				var m = JSON.parse(mjson);
				self.handleMessage(udpServer, m, {
					address: r.address,
					port: r.port
				});
			} catch (e) {
				if (debug.enabled) debug('%s UDP invalid packet %s %s (%s)', self.peer_name, r.address, r.port, e);
			}
		});
	...
	this.on('set', function (peer, k, v) {
		if (k.indexOf('event:')===0) {
			...
			self.emit('event:' + whichEvent, v, peer);
		}
	});
...
*/
Gossiper.prototype.gossipNow = function() {	
	if (this.gossipTimer)
...
/*	distribute an event by setting a local key with the 'event:' prefix.
...
*/
Gossiper.prototype.say = function (event, data, ttl /* in milliseconds*/, buffered) {
	//TODO add anonymous parameter?
	if (!ttl)
		ttl = eventDefaultTTL;
	var eventID = this.localEventPrefix + '_' + this.localEventNumber++;
	this.setLocal('event:' + event + ':' + eventID, data, Date.now() + ttl);	
	
	if (!buffered)
		this.gossipNow();
};

Gossiper.prototype.hear = function (event, callback) {
	this.on('event:' + event, function(value, peer) {
		this.event = this.event.split(':')[1];
		callback.apply(this, [value, peer]);
	});	
};

Gossiper.prototype.hearOnce = function (event, callback) {
	this.once('event:' + event, function(value, peer) {
		this.event = this.event.split(':')[1];
		callback.apply(this, [value, peer]);
	});			
};

Gossiper.prototype.know = function (key, callback) {
...
Gossiper.prototype.after = function (delayMS, f) {
	var self = this;
...
Gossiper.prototype.every = function (intervalMS, f) {
	var self = this;
...
Gossiper.prototype.sendUDP = function (m, address, port) {
	var mjson = JSON.stringify(m);
	var b = new Buffer(mjson, "utf8");
...
Gossiper.prototype.gossipToPeer = function (peer) {
	//if (debug.enabled) debug('gossip %s => %s', this.peer_name, peer.name)
...
Gossiper.prototype.setLocal = function (k, v, expiresAt) {
	this.my_state.updateLocal(k, v, expiresAt);
}

Gossiper.prototype.getLocal = function (k) {
	return this.my_state.getValue(k);
}

Gossiper.prototype.getRemoteKeys = function (peer) {
	return this.peers[peer].getKeys();
}

Gossiper.prototype.getRemote = function (peer, k) {
