...
var Gossiper = exports.Gossiper = function(port, seeds, address, emitUpdateOnHeartBeat) {
  ...
  this.peer_name = net.isIPv6(this.address) ? 
						['[' + this.address + ']', this.port.toString()].join(':') : 
						[this.address, this.port.toString()].join(':');
...
Gossiper.prototype.start = function(callback) {
  ...
  if (this.beatHeart)
    this.heartBeatTimer = setInterval(function() { self.my_state.beatHeart() }, heartbeatIntervalMS );

  this.gossipTimer = setInterval(function() { self.gossip() }, gossipIntervalMS );
}
