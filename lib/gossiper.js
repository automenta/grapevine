...
Gossiper.prototype.start = function(callback) {
  ...
  this.server.on('connection', function (socket) {
    ...
    socket.on('error', function (e) {
      debug('%s => %s error: %s' , socket.remoteAddress + socket.remotePort, e);
    })
  });
  ...
  // Bind to ip/port
  if (this.public) {
	//when address is specified, listen only accepts connections from that host
	self.server.listen(self.port, null, start);
  }
    ...
    this.server.listen(this.port, (this.public ? null : this.address), start);
  } else {
    ...
    child_process.exec('hostname', function(error, stdout, stderr) {
      ...
      dns.lookup(hostname, 4, function(err,address, family) {
        ...
        self.server.listen(self.port, address, start);
      });
...
Gossiper.prototype.gossipToPeer = function(peer) {
  ...
  gosipeeSocket.on('error', function(exception) {
    debug('%s => %s error: %s' , self.peer_name, peer.name, exception);
  });
}
