...
PeerState.prototype.setKey = function(k, v, n, ttl) {
  ...
  if (typeof(ttl) === 'number' && ttl < this.minTTLSeen) {
    if (debug.enabled) debug('updating minTTLSeen %s => %s', this.minTTLSeen, ttl)
    this.minTTLSeen = ttl
  }
...
PeerState.prototype.expireLocalKeys = function(now) {
  ...
  if (debug.enabled) debug('%s expireLocalKeys()', this.name);
...
PeerState.prototype.expireKey = function(k) {
  ...
  var self = this
  setImmediate(function () {
    if (debug.enabled) debug('%s: expiring %s', self.name, k)
...
PeerState.prototype.beatHeart = function() {
  ...
  this.updateLocal(PeerState.heartbeat, this.heart_beat_version);
}
...
PeerState.prototype.deltasAfterVersion = function(lowest_version) {
  ...
  for(k in this.attrs) {
    ...
    if(version > lowest_version) {
	  var a = [k,value,version];
	  if ((ttl!==null) && (ttl!==undefined)) a.push(ttl);
      deltas.push(a);
    }
  }
...
PeerState.prototype.isSuspect = function() {
  ...
  if ((this.detector.last_time==undefined) || (phi > this.PHI)) {
    this.markDead();
    return true;
  } else {
    this.markAlive();
