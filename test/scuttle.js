if(process.env['GOSSIP_COV']) {
  var Scuttle = require('../src-cov/scuttle').Scuttle;
  var PeerState = require('../src-cov/peer_state').PeerState;
} else {
  var Scuttle = require('../src/scuttle').Scuttle;
  var PeerState = require('../src/peer_state').PeerState;
}

var assert = require('chai').assert

suite('scuttle', function () {
  test('digest should have max versions we have seen', function () {
    ...
    assert.deepEqual({ 'a' : 10, 'b' : 12, 'c' : 22 }, scuttle.digest());
  })
})
//
//
